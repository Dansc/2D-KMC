...
c       parameters van systeem
	parameter (deltaEmax = 20.)

c       afgeleide variabelen
...
c-----------------------------------------------------------------------
	...
	do diffy = -16, 16
	 ...
	 Etemp = Etemp+Earray(PosLst(orientA,3,i),PosLst(orientB,3,j))
		endif

		if ( ((xi-xj).eq.0).AND.(abs(yi-yj).eq.1) ) then
	 Etemp = Etemp+Earray(PosLst(orientA,3,i),PosLst(orientB,3,j))
		endif	
		if ( (xi.eq.xj).AND.(yi.eq.yj) ) then
	 Etemp = Eoverlap
	 goto 5
		endif
		enddo !j
...
c 	Pos is bottom left lattice site
c      
...
c-----------------------------------------------------------------------
              ...
              Eold = Eold + Energy(posx-xold,posy-yold,orientA,orientB)
